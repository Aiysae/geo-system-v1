...
import type { AgentAuditRecord, AgentScope, AgentTokenRecord } from "@/types/agent"
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1.15fr)_minmax(360px,.85fr)]">
            <section className="rounded-lg border border-[#D8E7F7] bg-white p-5 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                <div><h2 className="text-sm font-bold">创建 Agent 密钥</h2><p className="mt-1 text-xs text-slate-500">建议从“业务执行”开始，后续按需重新创建更小权限的密钥。</p></div>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-3">
                ...
                <NumberField label="每分钟请求数" value={rateLimitPerMinute} onChange={setRateLimitPerMinute} min={1} max={600} />
              </div>
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <label className="text-xs font-semibold text-slate-700">IP 白名单（可选）<input value={allowedIps} onChange={event => setAllowedIps(event.target.value)} placeholder="多个 IP 用逗号分隔" className="mt-2 h-10 w-full rounded-lg border border-slate-200 px-3 text-xs font-normal outline-none focus:border-[#1677FF]" /></label>
              ...
              <button type="button" onClick={() => void createToken()} disabled={submitting} className="mt-5 inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-[#1677FF] to-[#00AEEA] px-4 text-xs font-bold text-white shadow-sm disabled:opacity-60">
