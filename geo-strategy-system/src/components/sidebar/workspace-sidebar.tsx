  ...
  return (
    <aside className={cn("no-print fixed inset-y-0 left-0 z-50 flex h-screen w-[248px] shrink-0 transform flex-col overflow-hidden bg-[linear-gradient(180deg,#001743_0%,#002C70_52%,#003B8F_100%)] text-white shadow-[10px_0_36px_-28px_rgba(0,29,102,.9)] transition-transform duration-300 ease-out md:static md:translate-x-0", drawerClass)}>
      ...
      <div className="shrink-0 px-3 pb-3 pt-4">
        ...
        <Link href="/account?tab=clients" className="group flex items-center gap-2.5 rounded-lg border border-white/12 bg-white/8 px-3 py-3 transition hover:bg-white/12">
          ...
          <span className="min-w-0 flex-1"><span className="block truncate text-xs font-semibold text-white">{client?.name || access.clientName || "选择客户"}</span><span className="mt-0.5 block truncate text-[10px] text-cyan-50/55">{client?.industry || (restricted ? "客户专属授权" : "进入我的主页切换")}</span></span>
      ...
      <nav className="min-h-0 flex-1 space-y-1 overflow-y-auto px-2 pb-4" aria-label="工作台功能模块">
        {DASHBOARD_MODULES.map(item => {
          ...
          const readOnly = restricted && item.key !== "penetration" && item.key !== "feedback"
          return (
