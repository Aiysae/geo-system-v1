...
function buildContentCalendar(
  ...
  const mediaPlan = Array.isArray(strategy.media_plan) ? strategy.media_plan : []
  const platforms = mediaPlan
    .map(item => text((item as Record<string, unknown>)?.platform))
    .filter(Boolean)
  const fallbackPlatforms = ["知乎", "小红书", "公众号", "百家号", "头条号", "B站专栏"]
  const channelPool = platforms.length > 0 ? Array.from(new Set(platforms)) : fallbackPlatforms
...
async function handler(req: NextRequest) {
  try {
    ...
    // 4. Build content calendar locally to avoid one more long LLM call.
    const contentCalendar = buildContentCalendar(reindexed, strategy)
