...
async function authorizeJob(input: {
  jobId: string
  userId: string
  action: "view" | "execute"
}) {
  const job = await getBackgroundJob(input.jobId, input.userId)
  if (!job) return { response: NextResponse.json({ error: "后台任务不存在或已过期" }, { status: 404 }) }
...
export async function GET(
  ...
  const authorized = await authorizeJob({ jobId, userId: userGuard.userId, action: "view" })
...
export async function PATCH(
  ...
  if (!userGuard.ok) return userGuard.response
  const { jobId } = await context.params
  const authorized = await authorizeJob({ jobId, userId: userGuard.userId, action: "execute" })
  if (authorized.response) return authorized.response
