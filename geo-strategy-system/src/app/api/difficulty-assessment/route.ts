...
function normalizeProcess(
  ...
  return stagesForMode(mode).reduce((acc, stage) => {
...
async function handler(req: NextRequest) {
  ...
  try {
    ...
    const mode: DifficultyAssessmentMode = body.mode === "brand" ? "brand" : "industry"
    const industry = text(body.industry)
    const city = text(body.city, "全国")
    const targetBrand = text(body.targetBrand ?? body.brandName)
    const website = text(body.website ?? body.brandWebsite)

    if (!industry) {
      return NextResponse.json({ error: "请填写行业/赛道名称" }, { status: 400 })
    }
    if (mode === "brand" && !targetBrand) {
      return NextResponse.json({ error: "请填写要评估的品牌名称" }, { status: 400 })
    }
    ...
    const context: StageContext = {
      ...
      city,
      target: mode === "brand"
        ? `${city && city !== "全国" ? city : "全国"} · ${industry} · ${targetBrand}`
        : city && city !== "全国" ? `${city}${industry}` : industry,
