...
const DEFINITIONS: AiProviderDefinition[] = [
  ...
  },
  {
    key: "article",
    label: "文章生成",
    description: "用于文章生成模块，可配置 DeepSeek 官方接口或 OpenAI 兼容中转站。",
    defaultBaseUrl: "https://api.deepseek.com",
    defaultChatPath: "/v1/chat/completions",
    defaultModel: "deepseek-chat",
    defaultTimeout: 600,
    apiKeyEnv: ["ARTICLE_API_KEY", "DEEPSEEK_API_KEY"],
    modelEnv: ["ARTICLE_MODEL", "DEEPSEEK_MODEL"],
    baseUrlEnv: ["ARTICLE_BASE_URL", "DEEPSEEK_BASE_URL"],
    urlEnv: ["ARTICLE_CHAT_URL", "DEEPSEEK_CHAT_URL"],
    chatPathEnv: ["ARTICLE_CHAT_PATH", "DEEPSEEK_CHAT_PATH"],
    timeoutEnv: ["ARTICLE_TIMEOUT"],
    presets: [
      {
        key: "article-deepseek-official",
        label: "DeepSeek 官方 · deepseek-chat",
        description: "默认文章生成配置，适合长文与结构化内容生产。",
      ...
      {
        key: "article-openai-compatible-relay",
        label: "中转站 · OpenAI 兼容",
        description: "选择后填写中转站根地址、Chat Path、API Key 和模型名。",
  ...
  },
  {
    key: "hunyuan",
    label: "腾讯元宝/混元",
    description: "渗透率严格检测使用腾讯 TokenHub HY3 官方联网搜索并返回结构化来源。",
    ...
    presets: [
      {
        ...
        label: "腾讯混元官方 · Turbos",
        description: "保留给非严格调用；疑问句严格联网检测请使用 TokenHub HY3。",
      ...
      },
      {
        ...
        description: "疑问句检测推荐配置，使用 TokenHub 官方 web_search 并返回 search_results。",
        baseUrl: "https://tokenhub.tencentmaas.com",
        chatPath: "/v1/chat/completions",
        model: "hy3-preview",
        extra: { enableEnhancement: true },
