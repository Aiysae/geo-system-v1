...
try {
  ...
  await saveActionReminderSettings(owner.id, {
    emailEnabled: false,
    inAppEnabled: true,
  })

  assert.deepEqual(await listEligibleActionReminderOwnerIds(), [owner.id])
  ...
  const first = await dispatchActionReminderForOwner(owner.id, reminderDate)
  const second = await dispatchActionReminderForOwner(owner.id, reminderDate)
  assert.equal(first.status, "sent")
  assert.equal(second.status, "sent")
  const notifications = await getUserNotificationSnapshot(owner.id, 20)
