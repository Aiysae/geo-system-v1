...
const { parsePublishingPlatformRecommendations } = contractModule.default || contractModule
...
const filtered = parsePublishingPlatformRecommendations(JSON.stringify({
  platforms: [
    ...
    { platform_key: "sohu", industry_fit: 70, stage_value: 65, recommended: true, reason: "首条。" },
    { platform_key: "sohu", industry_fit: 10, stage_value: 10, recommended: false, reason: "重复条目。" },
  ],
}), allowed)
assert.equal(filtered.length, 1)
assert.equal(filtered[0]?.reason, "首条。")
...
const qwenFallback = await recommendPublishingPlatformsWithAi({
  ...
  configured: async model => model === "qwen",
...
console.log("Publishing recommendation contract and two-stage recovery passed")
