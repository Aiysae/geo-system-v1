...
import type { AnalysisResult, OptimizeOptions, ApiSettings, OptimizeResult } from "@/types/aigc-detector"
import {
  Settings,
  Search,
  Sparkles,
...
export default function AigcDetectorPage() {
  ...
  const getScoreLabel = (score: number, inverse = false) => {
    const effectiveScore = inverse ? 100 - score : score
    if (effectiveScore >= 70) return "良好"
    if (effectiveScore >= 40) return "一般"
    return "较差"
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        {/* API Settings Panel */}
        {showSettings && (
          <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6 shadow-sm animate-[fade-in-up_0.3s_ease-out]">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--primary)]/20 focus:border-[var(--primary)]"
                >
                  {API_PROVIDERS.map((p) => (
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6 shadow-sm">
          ...
          <textarea
            ...
            className="w-full h-64 px-4 py-3 border border-slate-200 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-[var(--primary)]/20 focus:border-[var(--primary)] text-slate-700"
          />
          {error && (
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              ...
              <div className="bg-slate-50 rounded-xl p-4">
                ...
                <div className={cn("text-sm mt-2", getScoreColor(analysisResult.approvalScore))}>
                  {getScoreLabel(analysisResult.approvalScore)}
                </div>
              </div>
            </div>
        ...
        {/* Optimize Panel */}
        {analysisResult && (
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden animate-[fade-in-up_0.3s_ease-out]">
            <button
                ...
                <div className="flex justify-end mb-4">
                  <button
                    ...
                    className={cn(
                      ...
                      status === "optimizing"
                        ? "bg-slate-200 text-slate-400 cursor-not-allowed"
                        : "bg-gradient-to-r from-[var(--primary)] to-blue-600 text-white hover:shadow-lg hover:shadow-blue-500/20"
                    )}
                  >
                    {status === "optimizing" ? (
                      <>
